	...
	<resource>
	  ...
	  <value>
	    SELECT   U.USER_NO
		        ,U.USER_NM
		        ,U.EMP_NO
		        ,decrypt_char( U.PSWD,'ANAFINANCE') PSWD
		        ,U.USE_FG
	  ...
	  <value>
		...
		FROM
			...
			AND   C.ROLE_NO = D.ROLE_NO
		        AND   C.USER_NO = ?
		        AND   ( ((SUBSTR(C.USER_NO,1,3) = 'CAB' AND B.PRGM_NO LIKE 'P%') OR 
/*		2008년 10월 04일변경 내용 */
		              EXISTS(SELECT 1
								           FROM   GUSER.GBCT_COMM_CD_DESC AA
													 WHERE  AA.CD_KIND_NO = 'L50001'
													 AND    AA.CD_DESC_NO = B.PRGM_NO))	 
		           OR
                   	(SUBSTR(C.USER_NO,1,3) != 'CAB' AND   B.PRGM_NO NOT LIKE 'P%')
                  ) 
/*		2008년 10월 03일까지의 내용        
...
*/                  
		...
		AND   T2.MARK_FG = ? 
                ORDER BY CASE WHEN SUBSTR(T2.PRGM_NO, 1, 1) = 'P' THEN T2.MENU_REF_ODR ELSE 'G' || T2.MENU_REF_ODR END
		WITH UR
	  </value>
	</resource>
	<!--<resource>
